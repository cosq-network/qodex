...
`/model` shows the active provider, backend, and tool mode. Use `/model local` to browse downloaded local models, `/model groq` to browse Groq models, or `/model openrouter` to browse OpenRouter models. The picker supports arrow keys or `Ctrl+J`/`Ctrl+K`, type-to-filter, Enter to select, and Esc to cancel. Hosted model entries show `FREE`, `FREE TIER`, or pricing badges; OpenRouter `FREE` means all advertised pricing dimensions are zero, while Groq `FREE TIER` refers to provider free-plan quota access.
